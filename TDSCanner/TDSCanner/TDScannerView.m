...
- (void)setupUI{
    ...
    [self td_addScannerLineAnimation];
...
- (void)td_addScannerLineAnimation{
    ...
    //  重置动画运行速度为1.0
    self.scannerLine.layer.speed = 10;
}
...
- (UIButton *)flashlightBtn {
    if (!_flashlightBtn) {
        ...
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"lightNormal"] forState:UIControlStateNormal];
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"lighted"] forState:UIControlStateSelected];
    }
...
- (void)flashlightClicked:(UIButton *)button {
    ...
    [self td_setFlashlightOn:self.flashlightBtn.selected];
}
